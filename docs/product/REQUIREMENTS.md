...
- `agentic review plan --review-index <path> --json` creates a local human-readable Agentic Human Review plan and metadata package from an existing review artifact index. The plan explains the visual, UX, content, comprehension, subjective-reaction, risk, and improvement-review scope, selects reviewer roles by effort mode, computes a plan hash, and performs no provider execution.
- `agentic review run --plan <path> --plan-hash <hash> --execute --json` runs only a matching Agentic Human Review plan with the exact approved transfer flags. Results are written as `agentic_human_review_advisory`, remain advisory-only, and cannot mutate deterministic review findings, metrics, release gates, existing review artifacts, or MCP permissions.
