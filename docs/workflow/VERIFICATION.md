...
Agentic Human Review roadmap Slice 26-33 adds no-browser coverage for `agentic review plan/run/status/list`, effort-mode role orchestration, plan-hash validation, exact transfer-flag validation, tampered-plan rejection, fake provider advisory normalization, injected runner redaction, generic `agent execution` bypass rejection, schema registry/file parity, API exports, operation registry/capability exclusions, MCP execution-gate reporting, MCP tool non-exposure, architecture boundaries, product security checks, and packed-install file/API/schema/MCP coverage. Browser smoke tests are not required unless browser runtime behavior changes.
...
node ./bin/trace-cue.js agentic review plan --review-index .browser-debug/review-artifacts/<id>.json --effort standard --json
