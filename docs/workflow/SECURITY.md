...
- Keep content UX advisory manifest opt-in, advisory-only, local-only, bounded to inline source data, and limited to bounded review evidence summaries.
...
- Keep `local_content_ux_advisory` manifest opt-in and advisory-only; it may use bounded inline `sourceData` and bounded target-review element summaries in process but must not copy source values or full page text into advisory messages or Markdown reports.
